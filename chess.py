    ...
    if piece == 'P': 
        if color=='w': 
            if not isOccupied(board,x,y-1) and not AttackSearch:
                #The piece immediately above is empty fill it!
            ...
            while True:
                ...
                if kx<=7 and kx>=0: 
                    ...
                    if not isOccupied(board,kx,y):
                        #The square being looked at it empty
                    ...
                    break
        #Now using the same method for the vertical squares
        for i in [-1,1]:
            ky = y
            while True:
                ky = ky + i 
        ...
        #FOR THE MOVEMENT OF THE BISHOP....


    elif piece == 'B': # A bishop.
        #A bishop moves diagonally. This means a change in x is accompanied by a
        #change in y-coordinate when the piece moves.


        for dx in [-1,1]: #Allow two directions in x ad y and up and down etc..
            for dy in [-1,1]: 
                kx = x 
                ky = y
                while True: 
...
def makemove(position,x,y,x2,y2):
    ...
    #King:
    if piece == 'K':
        #Ensure that since a King is moved,
...
def drawBoard():
    ...
    if isTransition:
        #If a piece is being animated, the player info is changed 
...
isFlip = -1
AIPlayer = -1
#variable to keep false until the user wants to quit:
gameEnded = False
##################################        INFINITE LOOP        ##########################################
...
while not gameEnded:
    if isMenu:
        ...
        if isFlip!=-1:
            ...
            drawBoard()
            #menu won't appear again
            isMenu = False
           #call AI if player chooses
        ...
        continue
 # menu part complete 
 #highlight different boxes to show AI thinking for the next move
    numm+=1
    if isAIThink and numm%6==0:
        ...
        if ax%4==0:
            createShades([])
        #If the AI is white, start from the opposite side 
        if AIPlayer==0:
